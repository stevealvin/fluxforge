...
- **现代运行态与存储仪表盘 (Hero Dashboard)**：
  - 顶部设计 QuickJS-NG 沙箱微内核运行态指示灯（绿色呼吸微光），实时呈现内核就绪状态；
  - 集成真实本地缓存容量条与「一键瘦身」清理功能，点击后异步清空图片与网络临时目录，释放本地磁盘；
  - 呈现「已启用规则」、「媒体消费资产」与「沙箱自愈诊断」核心资产三联统计磁贴；
  - **严格去除虚假延迟显示**：坚决移除原型中缺乏真实每秒探测基础的模拟延时（如 168ms），升级为真实的本地数据自治与全链路健康诊断。
- **四大核心场景专区磁贴（2x2 响应式场景卡片）**：
  - 🎬 **影音与播放**：卡片实时展示加速倍速与起播方式，点击滑出专属配置抽屉（支持长按瞬时加速开关整行可点、加速倍率下拉、断点续播行为下拉）；
  - 📖 **阅读与排版**：卡片实时展示小说与漫画阅读偏好，点击滑出专属配置抽屉（小说平滑横翻/上下滚动、漫画左右翻页/长条连读同源持久化保存）；
  - 🎨 **外观与风格**：卡片实时展示当前主题外观，点击直达「选择系统主题外观」抽屉，支持纯净星暮白、曜夜极光翡翠与跟随系统秒级切换；
  - 🛡️ **网络与安全**：卡片展示沙箱超时与广告拦截状态，点击滑出专属配置抽屉（15s/30s/60s 超时下拉、网页广告拦截开关与热更状态面板、自定义 UA 面板、启动规则同步）。
- **收敛的高级与系统诊断工具箱**：
  - 收纳「数据备份与还原」、「沙箱与系统日志中心」（带实时 ERROR 徽标）、「关于 FluxForge」应用概览；
  - 保留「恢复默认偏好」并遵循全仓统一的二次确认弹窗，出厂重置仅影响偏好参数，资产数据安全留存。
- **质量验证**：
  - `flutter analyze`：**0 错误、0 警告 (No issues found!)**；
  - `flutter test`：**531 项自动化测试全量通过 (531/531 All tests passed!)**，专门编写 6 项组件测试覆盖仪表盘、专区抽屉交互、同源持久化与重置流程。

### 📖 代码全面审查与长图模式定位收敛

- **设置页重构与统一行组件规范**：
  - 抽出通用 `SettingSection` 与 `SettingRow`，容器统筹边框与分隔线，行组件对齐视觉规范；
  - 梳理为「播放与渲染 / 浏览与阅读 / 网络与规则 / 外观 / 数据与诊断」5 大清晰分组，`SettingSectionTitle` 增加分组说明（`subtitle`）；
  - 新增「恢复默认偏好」并带二次确认对话框，仅重置设置页本身偏好，不影响收藏、历史和下载；
  - 抽取 `ComicReaderPreferences` 实现设置页与漫画阅读器读写同一把持久化 Key，杜绝设置与临时切换状态脱节。
- **收藏与发现页交互降噪**：
  - 发现页卡片移除左上角红心角标与长按收藏，卡片职责回归纯粹的「点击进入详情」；
  - 收藏页调整为每行 3 个黄金比例网格（宽高比 0.47），卡片长按改为底部动作面板（仅展示「移除收藏」并保留 5 秒撤销），Shell 底部导航栏上的「收藏」Tab 图标取消红点提示。
- **详情页头部 `MediaMetaHeader` 空间结构优化**：
  - 评分与规则来源上移贴至封面角标，题材标签紧跟标题，扫读路径更顺；
  - 新增 `bottomAction` 插槽，将小说与漫画的「继续阅读 / 开始阅读」主按钮直接嵌入头部右列底部，移除原来下方多余的大卡片按钮；
  - 简介改为平铺段落（`_buildDescSection`），绿色主题竖线标识 + 小标题 + 整体可展开收起。
- **漫画长图模式定位修复与收敛**：
  - 修复长漫阅读器在冷启动（如按 `initialIndex` 恢复）或切换阅读模式首帧时，控制器尚未挂载（`!_scrollController.hasClients`）直接退出导致定位静默失败的问题，改为等待帧后自适应重试收敛；
  - 消除 `use_build_context_synchronously` 静态分析警告。
- **质量验证**：
  - `flutter analyze`：**0 错误、0 警告 (No issues found!)**；
