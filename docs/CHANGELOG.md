...
**现象**：图片类详情页的「图集画卷」标题写着「共 N 张」，网格却固定只画 9 个
（`imageList.length.clamp(0, 9)`），且 `physics: NeverScrollableScrollPhysics` ——
标题与内容自相矛盾，多出来的图片**没有任何入口**能看到（只能进阅读器逐张翻）。

**修复**：`itemCount` 取全量；网格改为**固定高度 + 独立滚动窗口**（约两行高、
`ClampingScrollPhysics`）—— 既能看到全部，又不会把详情页撑到极长。

窗口高度按**屏宽反算**（`_imageGridHeight`）而不是写死：网格是 3 列、单格 4:3
（`childAspectRatio: 0.75`），行高由列宽决定 —— 写死高度会在窄屏多露一行、宽屏少露一行。
同时去掉 `shrinkWrap`（有确定高度后它只会让布局多算一遍）。

**已知取舍**：图集窗口与外层详情页是**同向嵌套滚动**，手指在窗口内滑到底不会带动外层
页面 —— 这是 Flutter 嵌套滚动的固有行为，要打通需引入 `NestedScrollView`。此处图集的
用途是「看小图挑一张」，局部滚动更顺手，故保持现状。

**验收**：新增 `comic_detail_view_test.dart`（该视图此前无测试）覆盖「12 张全量渲染
（断言网格的 `itemCount` 而非可见 widget 数 —— 视口外的格子不会构建）」「physics 不再是
`NeverScrollableScrollPhysics`」「只有 3 张时同样正常渲染」。`flutter analyze` **0 问题**；
`flutter test` **519/519 通过**。

### 🔧 收藏页每行 4 个；修复规则发现页分类条「滚动越界」

**收藏页**：网格由每行 2 个改为 **4 个**（`crossAxisCount: 4`），封面仍锁 2:3 比例，
高宽比相应调整为 0.42；单元格变窄后标题保留两行、超出省略。

**规则发现页（`RuleCatalogPage`）分类条滚动**：原实现用 `index * 72.0 - 80.0`
**估算**选中 chip 的位置，但 chip 宽度随标题长度变化（「玄幻」≈48px 与「最近更新」≈72px
差着一倍），估算一旦偏大就会**滚过头** —— 用户看到的正是「点靠后的分类，分类条往前窜一截、
选中的那个还跑到视口外」；另外 `- 80` 会把前两个下标都压到 offset 0。

现改为：每个 chip 挂稳定 key（按「标题|地址」记忆），用
`Scrollable.ensureVisible(..., alignment: 0.5)` 按**真实布局**居中 —— 自带边界处理，
不必再手动 clamp。chip 不在已构建范围内时（例如从展开面板里点选）直接跳过。

顺带一处显式化：切换分类时把内容列表归零到顶部 —— 换分类等于换了一批内容，停在上一批
的滚动位置没有意义；此前是否归零取决于中间态高度，属未定义行为。

**验收**：`flutter analyze` **0 问题**；`flutter test` **516/516 通过**。
分类条滚动这一处**未加自动化测试**：该页的分类来自 `RuleEngine.discovery`（QuickJS 沙箱），
测试环境无 FFI C Bridge、拿不到任何分类 chip，无法构造断言场景；且修复后此处已无自算逻辑
（直接委托 `ensureVisible`），确无可单测的纯函数。

### 📚 收藏页改为「书架式」双列封面网格

**形态**：由「横向列表卡（封面 66×88）」改为**双列封面网格** —— 封面占满单元格宽度、
2:3 海报比例，成为识别物的主体；下方依次是标题、`上次看到`、以及有更新时的 `更新至`。
一屏约 6 条。与「发现」页网格的边界在此定清：**发现页是探索（无状态），收藏页是我的
书架（带进度与更新标记）** —— 这正是 `APP_TODO` 第 21 条 ① 悬置未决的那个取舍。

**更新信号从三处收敛到两处（分工不同）**：此前是封面左竖条 + 标题右侧
`NEW · 第24集` 胶囊 + 「最新更新」行转主色，三处在说同一件事，扫列表时反而是噪声。
现在：封面左上角 `NEW` 角标（扫列表第一眼可见，随封面左上圆角做切角）+ 下方
`更新至：第24集`（读得出具体集数）。**无更新时不渲染这一行** —— 源站最新集在没有
新内容时没有信息量，省下的高度还给封面。

**移除收藏改成长按**：网格单元格里放不下垃圾桶按钮，且左上 / 左下角已被角标占用。
它仍是低风险操作，故照旧走「可撤销」而不是二次确认。

**网格列数按最大单元宽度决定**（`SliverGridDelegateWithMaxCrossAxisExtent`，上限 200）：
手机竖屏恰好两列，平板 / 横屏自动多一列，不必写设备判断。卡片内封面用 `Expanded`
吃掉文字之外的余量，所以标题占 1 行还是 2 行都不会溢出。

**验收**：`favorites_page_test.dart` 三处随改版同步（角标与集数分别断言、移除改长按、
「置顶」断言补上列比较 —— 网格里只有两个条目时它们必然并排，只比 dy 会恒假）。
`flutter analyze` **0 问题**；`flutter test` **516/516 通过**。

### 🔖 收藏入口迁到底部导航栏：新增「收藏」Tab，更新红点随入口走

**入口迁移**：收藏此前是首页顶栏的一个图标按钮（`pushFavorites`）。现改为底部导航栏的
**独立 Tab**（顺序：发现 / **收藏** / 规则 / 站点 / 我的）—— 收藏属「消费」场景，与
「发现」同族，故放在第二位；底部栏常驻可达，比顶栏图标少一次「先回首页」。

**「有新更新」红点跟着入口走**：原挂在顶栏入口上、由 `favorites.any((f) => f.hasUpdate)`
驱动的红点，改为「收藏」Tab 图标上的 `Badge` —— 迁移途中若丢掉它，「有更新」就再无提示。

**同一份数据不设两个入口**：首页顶栏的收藏按钮**已移除**（沿用「我的」页移除收藏卡时
定下的同一条原则）。发现页的**长按直接收藏**与卡片左上角「已收藏」角标保留不动 ——
那是「加入收藏」这个动作，不是入口。

**顺带消除一处魔法下标**：`ProfilePage.onSwitchTab(int index)` 改为
`onSwitchToRules`（`VoidCallback`），由外壳决定跳哪个下标。原写法在「我的」页写死
`call(1)` 表示规则 Tab，而本次在「发现」后插入「收藏」后规则变成 2 —— 这类错误
**编译器不会报**，只会静默跳到错误的页。外壳侧同时把被跨组件引用的下标收进 `_ShellTab` 常量。

**收藏页适配 Tab 形态**：列表的显式 padding 会**覆盖**滚动组件的自动 padding，作为 Tab
之后最后一条收藏会被毛玻璃底部栏压住 —— 现补上 `8 + MediaQuery.paddingOf(context).bottom`
（`extendBody` 时 Scaffold 已把底部栏高度注入该值，无需硬编码 64）。AppBar 不必改：
`automaticallyImplyLeading` 会按 `canPop` 自适应 —— Tab 形态无返回键，路由 push 形态仍有。

**验收**：新增 `shell_page_test.dart`（外壳此前**无测试**）覆盖「Tab 顺序 / 点「收藏」切到
收藏页 / 顶栏不再有收藏入口（限定在 AppBar 内查找，避免把 Tab 图标误判成顶栏入口）」。
`flutter analyze` **0 问题**；`flutter test` **516/516 通过**。

### ⚙️ 设置页样式重设计：抽出 SettingRow，五张卡统一外壳与行样式

**背景**：设置页是「配置中枢」，5 张卡此前各写各的 —— 内边距 / 圆角 / 标题行各不相同，
卡内行是手写 `ListTile`（图标是裸 Icon、没有徽章），每行之后还要手写一条 `Divider`。
同一套视觉语言在设置页与「我的」页各维护一份，必然漂移。

**统一为「分组标题 + 容器 + 行」三件套**（`shared/widgets/setting_tile.dart`）：

| 组件 | 职责 |
|---|---|
| `SettingSectionTitle` | 分组标题 —— 移到**卡片外**，卡内不再自绘小标题 |
| `SettingSection` | 卡片容器（圆角 18 + 细边框 + 微阴影），**自动**为相邻子项插入分隔线 |
| `SettingRow` | 行：图标徽章 38×38 / 圆角 11 / 14% 语义色底、标题 14 w600、说明 11.5 muted、**整行可点**、**数值右置**、`trailing` 留给开关 / 下拉 / 徽标 |

**右箭头只由 `onTap` 决定**：行可点就给箭头；开关与下拉自身即反馈，故不给（它们用
`trailing` 表达，`onTap` 仍可传入以实现「整行可点」）。日志行的「异常徽标 + 箭头」用
导出的 `SettingRowChevron` 复用同一图标，避免尺寸 / 颜色再次分叉。

**「我的」页同步切到 `SettingRow`**，删除页面私有的 `_ProfileActionRow`。

**顺带**：

- 分隔线缩进 56 → **64**（14 行内边距 + 38 徽章 + 12 间距），对齐行内文字起点；
- 语义色收敛：`Colors.amber` / `Colors.green` / `Colors.blueAccent` / `Colors.grey`
  与硬编码 `Color(0xFFF59E0B)`、`Color(0xFF8B5CF6)` 全部改用 `AppColors.*`
  （`accentAmber` / `success` / `accentBlue` / `accentPurple` / `muted`）；
- 主题选择面板的选项也改用 `SettingRow`，选中态**只留一个勾** —— 模式名已写明是哪种，
  再染色 / 加粗是对同一信息的重复强调（与「首页规则卡」那次收敛同一理由）；
- 广告拦截的状态块底色改用「比卡片深 / 亮一档」的内嵌块色（`lightSurfaceVariant` /
  `darkSurface`），去掉与容器重复的那圈边框；
- 当前值已由尾部下拉表达的行，说明文案改为写「这行是干什么的」（如「断点续播行为」
  原先重复显示当前策略值）。

**破坏性动作**：设置页当前没有破坏性行（日志清空在 `logs_page` 内、且已走确认弹窗），
故未给 `SettingRow` 单独的 `danger` 参数 —— 传 `color: AppColors.danger` 即可表达。
`logs_page` 内 3 处裸色（ERROR / WARN 筛选 chip）属另一页样式，不在本轮范围。

**验收**：新增 `settings_page_test.dart`（该页此前**无测试**）覆盖「5 组标题 / 全部行是
`SettingRow` / 不再有 `ListTile` 与 `SwitchListTile` / 开关行整行可点 / 主题面板可打开」；
新增 `setting_tile_test.dart` 覆盖行组件的箭头规则、数值右置、busy 态与分隔线条数。
`flutter analyze` **0 问题**；`flutter test` **513/513 通过**。

**踩坑记录**（供后续给该页写测试参考）：设置页是懒加载列表，默认 600 高的测试视口只
构建前 3 组，断言后两组会误报「不存在」，需放大视口；`AppStorage` 基于
`SharedPreferencesAsync`，`SharedPreferences.setMockInitialValues` 对它是无效的，
必须设 `SharedPreferencesAsyncPlatform.instance`。

### 📦 下载列表显示总大小（已下载 / 预计总量）

**显示**：下载管理页任务卡给出 `1.2 GB / 2.4 GB`（已下载 / 预计总量）；
总量探测不到时**只显示已下载体积**，不编造分母。

**探测**：新增 `DownloadService.estimateTotalSize(id)`，复用既有 `probeUnitSizes`
（逐项 HEAD 取 `Content-Length`），且**只在每一项都拿到长度时**才写回
`DownloadTask.totalBytes` —— 部分未知时求和只是下限，而列表里没有「以上」这类修饰位
（下载面板的选集预估有），显示成分母会误导。`totalBytes` 入库持久化（探测成本高，
重启沿用），续传时沿用旧值。

**触发时机挂在展示侧**：探测在**打开下载列表**那一刻发起
（`DownloadManagerPage._refreshSize`），而**不是**创建任务时 —— 否则每个任务一入队就多一轮
HEAD，下载本身不值当为显示付这个成本。这一点是被既有端到端测试逼出来的：把探测放进
`_startTask` 后，`download_service_url_refresh_test` 的请求计数立刻从 1 变 2。
服务侧用 `_sizeProbed` 做内存去重，反复打开列表不会重复探测。

**能力边界**：`probeUnitSizes` 只对单文件直链有效 —— HLS 需逐片询问（其注释已明确
「成本远高于收益」）与「需再解析一层」的漫画章节都返回 `null`，此时退化为只显示已下载。
若要 HLS 也有总大小，可扩展 HLS 解析器读 master 的 `BANDWIDTH` 与 `#EXTINF` 时长做
码率估算（约数，需标注「约」）。

**验收**：新增 2 条单测（总量已知给出分母 / 未知不编造分母）；`flutter analyze` **0 问题**；
`flutter test` **489/489 通过**（含修回被探测请求干扰的 3 条既有用例）。

### ⚡ 下载列表显示实时网速

**位置**：下载管理页的任务卡与详情页下载面板的状态条，进行中显示
`12 / 40 章 · 2.4 MB/s`（暂停 / 完成后不显示 —— 那是过期值，留着会让人以为还在跑）。

**采样口径**：新增 `DownloadRateMeter`（纯逻辑，可单测）按「区间字节 ÷ 区间时长」估算，
默认 **800ms** 一个窗口。这个值同时承担两件事：**节流**（否则下载时列表每秒重建几十次）
与**窗口前移**（速率不会被整段历史拖住，停下后数字立刻掉下来）。

**三条下行通道统一入口** `DownloadService._reportBytes(taskId, delta)`：

| 通道 | 字节来源 | 粒度 |
|---|---|---|
| HLS 分片 | 分片落盘后按文件大小 | 分片级 |
| 直链流 | `await for (chunk)` **逐块上报** | 字节级（最细） |
| FFmpeg 合流 | `Statistics.getSize()` 是累计值 → 取增量（放在进度节流之前，两者口径不同） | 统计回调级 |
| 漫画图片 | 无进度回调 → 按落盘大小一次性上报 | 单图级 |

**瞬时状态不入库**：`DownloadTask.bytesPerSecond` 不写 `toJson`、`fromJson` 一律归 0
（与 `activeItemProgress` 同一约定）；暂停时清零；UI 仅在 `isActive` 时展示。

**覆盖边界**：小说章节走规则引擎抓正文（文本小、无字节级进度），暂未接入。

**验收**：新增 7 条单测（采样节流 / 区间速率 / 窗口前移 / 非正增量忽略 / reset 重新计时
+ 速率格式化两档）；`flutter analyze` **0 问题**；`flutter test` **487/487 通过**。

### 🎬 修复：视频下载请求头缺失与漏兜底（现象：**能播放，但点下载立刻失败**）

**现象**：同一集在详情页能正常播放，点下载却立刻失败 —— 这条对比正是根因的指路牌。

**根因（两层，都在下载侧）**：

1. **完全没传**：详情页解析时注入了 `Referer`（`MediaDetailPage._parseSandboxResult`），
   但创建任务时没有把 `customHeaders` 传给 `startVideoDownload` —— `task.headers` 为空，
   m3u8 清单与分片请求都不带头（漫画一直传了；小说走正文抓取，是另一条路径）；
2. **传了也不够**：播放器并非直接用 `data.customHeaders`，而是经
   `MediaRequestHeaders.withDefaults(...)` 补齐「**站点根** Referer + UA」。直链不走规则引擎，
   图床 / CDN 通常只认站点根，用详情页的深层地址当 Referer 同样被判盗链。

**修复**：新增 `MediaDownloadActions.downloadHeadersOf(data, rule:, episodeExtra:)`，
按「详情级 → 单集独占头 → 防盗链兜底」组装，**与播放器同源**；视频与漫画下载都改用它
（漫画此前也缺兜底）。任务级只有一份头，单集鉴权头取首集（同线路各集实践中一致）。
下载侧本就使用 `task.headers`（清单 / 分片 / FFmpeg 合流均透传），补上头即可。

**验收**：新增 3 条护栏测试（站点根 Referer 与 UA 兜底、显式声明不被兜底覆盖、
单集独占头并入任务头）；`flutter analyze` **0 问题**；`flutter test` **480/480 通过**。

### 🖼️ 漫画阅读器：底部栏与「阅读方式」弹窗瘦身，取消选中态染色

**底部控制栏降高**（原约占屏近三成）：

- 页码并入进度行（原先独占一行，与小说阅读器同构的写法），少一行、降约 20px；
- 内边距收紧（上 10→6、下 +12→+8）、底部按钮内边距 8→6、行间距 4→2。

**「阅读方式」弹窗降高**：选项由「带副标题的 `ListTile`」改为**紧凑单行**
（图标 + 模式名 + 说明同行，说明单行省略），标题与底部留白同步收紧 ——
单项高度从 ~56 降到 ~40。

**「长漫画」不再有任何额外样式**（绿色 → 加粗 → 全部去掉）：

- 底部栏按钮：不因处于长漫画而改色或改字重，`highlighted` 参数直接移除 ——
  label 本身（「长漫画」/「左右翻页」）已写明当前模式，任何额外强调都是重复；
- 弹窗选项：图标与模式名用常规文本色、常规字重，仅保留末尾一个勾作为「当前项」标记。

**验收**：测试同步更新（选项已非 `ListTile`，改用 `ValueKey('reading_mode_<bool>')` 定位）；
`flutter analyze` **0 问题**；`flutter test` **477/477 通过**。

### 🖼️ 图片阅读器：图片占位页高度统一（加载中 / 失败不再两种高度）

**现象**：连续滚动（长漫画）模式下，图片「加载中」占位高 **280**、「加载失败」占位高 **220** ——
状态切换时列表项高度突变，滚动位置抖动、画面跳动；两者底色也不同（纯黑 / 深灰），
切换瞬间还会显出一块色块。

**修复**：抽出 `_buildImagePlaceholder` / `_buildLoadingIndicator` / `_buildRetryPrompt`
三个共用实现，加载中与失败**共用同一高度（`kComicPagePlaceholderHeight`）、同一底色**
（取 Scaffold 背景色）、同一指示器与重试按钮尺寸；翻页模式复用同一实现（撑满视口，不设高度）。

**验收**：新增护栏测试「连续模式下图片占位页高度唯一」（断言占位高度集合只有常量一个值，
排除撑满父级的无限约束）；`flutter analyze` **0 问题**；`flutter test` **476/476 通过**。

### 🗂️ 下载面板：选集改为网格卡片，「下载全部」移到「下载选中」旁

**选集网格化**：原先是一个大容器套若干「系统 `Checkbox` + 文字」行；现改为**紧凑网格** ——
密度对齐漫画章节目录（格子高 40、间距 8、圆角 8，列数仍按可用宽度自适应），
每格只放标题、自带点击热区。**未选中不画边框线**：用透明边框占位而不置 `null`，
这样切到选中态（主色描边出现）时内容不会位移。

**底色分档**：未选中格子此前与面板同色 —— 亮色面板是纯白，**白格子铺白底又去掉边框，
区块等于没有边界**。现改用「比面板深 / 亮一档」的内嵌块底色：亮色取新增的
`AppColors.lightSurfaceVariant`（Slate 100，同时把浅色主题里硬编码的同一个值收敛到该常量），
暗色沿用 `darkCard`。于是不画边框也有清晰的区块感。

**选中态以颜色边框为主**：主色描边 + 主色微底 + 主色加粗标题；边框在**加色**的同时**加粗**
（0.8 → 1.4）—— 颜色之外还有一处可辨差异，色弱用户同样分得清。

### 🎬 视频选集：去掉未选中态的默认边框线

视频详情页的横向选集条与「全部选集」弹窗，此前每个格子都画一条 0.8px 默认边框；
底色本身已足以区分边界，这条线只是视觉噪声。现改为**未选中不画边框**
（弹窗沿用透明边框占位、样式与下载面板同源），只有当前集亮主色描边。

**「下载全部」的位置**：从状态条里的 idle 文案提为独立按钮，放在**「下载选中」右侧**（底部动作行），
不放在「清空」后面。理由：

- **语义同组**：「下载选中 / 下载全部」都是*发起下载*，只是范围不同；「清空」属于*选区编辑*，
  与「全选」同族 —— 放进同一行会让「清空」读起来像下载的前置步骤；
- **拇指热区**：两者都是提交动作，归底部主操作区；混进顶部工具栏还会紧邻「清空」造成误触；
- **主次分明**：主按钮 `FilledButton`（下载选中）+ 次按钮 `OutlinedButton`（下载全部）。

**配套**：状态条（`DownloadBar`）改为**仅在已有任务时渲染** —— 未开始时入口由动作行承担，
避免同一动作出现两个入口（下载中 / 暂停 / 重试仍由状态条承载）。

**顺带修复**：面板内容变多后在窄屏上 `RenderFlex` 溢出（测试环境即复现），外层改为可滚动。

**验收**：测试同步更新（系统 `Checkbox` 定位改为卡片 key），并新增 2 条护栏
（选集为网格布局且选中态以主色边框加粗表达、已有任务时「下载全部」入口不重复出现）。
...
**改动**：`SearchPage` 的 `static const _hotSuggestions` 移入 `SearchHistoryPanel`，
并**随之下掉该入参** —— 全仓仅一处构造点、无测试引用，宿主不必为一个纯静态常量
多维护一个参数。常量与宿主的归属也随之对齐：它是该面板自身的展示内容。

`search_page.dart` **328 → 322 行**（-6）；`search_history_panel.dart` 210 → 214 行（+4，
常量与注释随之迁移）。两文件净 -2 行，但门禁按**单文件**判定：页面仍为 322 行、**仍超阈值**。

**结项说明（其余两项决定不做）**：原条目估「三项拆完即可从白名单销账」，量化后**达不到**，
故不再单独立条目，债务统一并入 APP_TODO 第 16 条排队：

| 原可拆项 | 实际可省 | 结论 |
|---|---|---|
| `_hotSuggestions` 内聚进面板 | 6 行 | ✅ 本次已做 |
| 向 `SearchResultsView` 注入 `SearchSession` | 4~6 行（原文估 8~10） | ❌ 不做 |
| `SearchAppBar` 11 个入参合并为配置对象 | 页面侧 ≈ 0 | ❌ 不做 |

- 第 2 项估不准的原因：能下沉的只有**数据类**实参（`results` / `statusMap` / `isLoading` /
  `isLoadingMore` / `hasMore`），而 `onRefresh` / `onRetry` / `onDebugRule` / `onGoMarket`
  依赖页面的 `_performSearch`（内含 snackbar、焦点释放、历史落库），**搬不走**；
  代价却是把纯展示视图耦合到 `SearchSession`。
- 第 3 项省的是**组件侧**行数，页面照样要构造 11 个字段，对页面行数为零。

要真正销账需再削 22 行，只能靠更大的重构（编排逻辑下沉），而那正是 APP_TODO 第 19 条
已判定「收益为负」的方向 —— 故白名单条目保留。

**验收**：`flutter analyze` 0 问题；`flutter test` **277/277 通过**；
`dart run tool/guardrails/check_architecture.dart` 通过。

### 🧩 阅读器：缓存口径统一（切片与正文同生共死）+ 邻居分页延后到帧末

#### ① 先实测：真正贵的是分页，不是读盘

临时基准（4810 字符 ≈ 单章量级 → 17 页）：

```
单章分页（TextPainter 二分）: 71.25 ms     ← 主要开销
纯文本处理（清洗量级）      :  0.299 ms     ← 可忽略
```

由此澄清两件事：

- **纯顺序阅读时，正文缓存的收益接近于零** —— 每章只读一次，保留量一次都不会被访问；
  它只在「回看 / 目录跳章 / 横纵切换」时兑现，而挡下的几乎只是 0.3ms 的读盘；
- 真正值得保留的是**分页结果**（`_chapterSlices`），因为重新分页要 71ms。

#### ② 缓存口径统一：切片与正文同生共死（修正上一版的 ±2 错配）

上一版把 `_chapterSlices` 裁到「当前章 ±2」，与「正文保留 200 章」组合成**错配**：
命中正文却命中不了切片，回看超过 2 章照样要重新分页（71ms）——
留了 1.2 MB 却挡不住主要开销。现改为：

- `ChapterCache` 容量 **200 → 31（±15）**：内存恒定约 **0.4 MB**（正文 + 切片合计），
  不随阅读时长增长，覆盖绝大多数回看与小范围跳章；
- **切片跟随正文一起被淘汰**（挂在 `_applyCacheEviction` 内）：
  命中正文即命中切片，保留半径内回看零开销；超出半径则两者一起重建
  （读盘 0.3ms + 分页 71ms@测试 VM）；
- 删除上一版的 `_pruneChapterSlices()` 与 `_sliceKeepRadius`。

口径关系就此固定：**渲染窗口管「屏上几章」，缓存管「回看缓冲」；
同一段数据的两份表示（正文 / 切片）必须同生共死。**

#### ③ 邻居分页延后到帧末

`_recalculatePages()` 末尾原先**同步**补齐前后各一章的分片，三章叠加会让「切章那一帧」
卡住上百毫秒（实测 71ms/章）。现改为 `_scheduleNeighborSlices()`：

- 当前章仍同步分片（本帧就要渲染）；
- 邻居章推迟到帧末（`addPostFrameCallback`，同一帧内多次请求合并为一次），
  分片补齐若改变滑窗总页数（占位页 → 真实页数）再触发一次重建。

邻居分片只需在用户真正翻到之前就绪（读一页至少几百毫秒），
因此既不阻塞切章首帧，也不影响跨章翻页的「零占位页」体验。

**验收**：`flutter analyze` **0 问题**；`flutter test` **277/277 通过**
（含「连续跨章翻页 5 次」的回归用例，防切片生命周期与分页延后影响阅读）。

### 🪟 长卷窗口化（原 APP_TODO 第 1 条 · 方案 B）+ 屏中线同步降频（原第 3 条）

#### ① 长卷窗口化：给纵向序列与正文缓存设上限

**背景**：`_verticalSequence` 只增不减（向下追加 + 向上前插），读过的章节全部留在长卷里。
更关键的是 `ChapterCache` 的保护集合包含**整条长卷**（不保护会出现空白块），
于是容量上限（200 章）在纵向模式下形同虚设 —— 保护集合只增不减，
**内存随阅读时长无上限增长**，这正是原条目所指的「纵向长卷内存无上限」。

**方案 B（完整窗口化）**：当前章前后各保留 5 章，其余摘除。
新增 `VerticalFlowEngine.resolveWindowTrim()`（纯逻辑）+ `VerticalWindowTrim` 结果对象，配套 **8 个单测**。

**为什么摘除是零成本的**：长卷本就是 `CustomScrollView.center` 锚点结构 ——
锚点及其以下内容的布局坐标独立于锚点之上的内容；再叠加「只摘除远离当前章、
已不在视口内的章节」，用户当前看到的位置不会移动，**无需任何滚动偏移补偿**
（这也是原条目判定方案 B「无损可行」的依据）。

**两条安全边界（都已写成单测）**：

| 边界 | 不遵守的后果 |
|---|---|
| 锚点章节**永不摘除** | 它是 Viewport 的坐标原点，摘掉会让滚动位置被重置 |
| 首端不得越过锚点、末端**不得吞掉**锚点 | 用户向上读到锚点之上时，锚点及其之后的内容整体位于视口下方、看似「可以摘」，实际摘掉同样毁掉坐标原点 |

第二条是写测试时才发现的：最初的实现只约束了首端，末端在「当前章在锚点之上」的场景下会把锚点裁掉。

**第三条边界（本轮补丁）**：窗口裁剪**只在「锚点之外」是免费的** ——
锚点与当前章之间的「中间区段」位于锚点之下、视口之上，摘除它会让下方内容整体上移，
而这些块多半已被 `SliverList` 回收（测不到高度）无法补偿偏移。
于是**向前连续阅读时（最常见的读法）序列依旧一直增长**，保护集合随之膨胀。

补丁：新增 `VerticalFlowEngine.needsReanchor()`（锚点漂移超过窗口半径即触发）+
`_reanchorVerticalFlowIfNeeded()` —— 把锚点移到当前章，中间区段立刻变成「锚点之上」，
摘除重新归零成本。偏移补偿取当前章块的实测几何（`pixels - ChapterMetrics.top`），
并与 `setState` 在**同一同步块内** `jumpTo`：本帧尚未构建与布局，因此不会画出中间态，视觉无感。

**收口点**：序列收缩后保护集合随之变小，`_evictChapterCacheIfNeeded()` 才真正按容量上限回收
—— 长卷模式下的内存上限由此生效（此前保护集合包含整条只增不减的长卷，
`ChapterCache` 的 200 章容量在纵向模式下形同虚设）。

#### ② 屏中线同步降频到每 4 帧 + 滚动停止补一次精确同步

`_onVerticalScroll` 原先每帧调用 `_syncVerticalCurrentChapter()`，内部做 `findRenderObject` +
`localToGlobal` 的屏中线判定。现改为每 4 帧一次（章号最多滞后约 50ms），
并由 `ReaderVerticalScrollView` 新增的 `onScrollEnd` 回调（内部监听 `ScrollEndNotification`）
在滚动停止时精确补一次，保证章号、目录高亮与进度条不与画面错位。

**验收**：`flutter analyze` **0 问题**；`flutter test` **276/276 通过**
（+15：10 个窗口裁剪单测（含「中间区段无法裁剪」与「重设锚点后收敛」两条缺口回归）、
4 个锚点漂移判定单测、1 个滚动停止回调用例）。

#### 废弃：原第 4、6 条

- **第 4 条（长卷 `ListView` 无 `itemExtent`）**：章节高度不一，`itemExtent` 本就不适用；
  彻底解决需自研虚拟滚动，**收益不匹配成本**，与条目自身的结论一致。
...
**背景**：该开关**从未被播放器读取** —— `AuraPlayer` 的手势层（垂直调光 / 调音、水平寻道）
始终生效，关闭它不产生任何效果（原 APP_TODO 第 13 条记载）。

**处置**：选择**删除设置项**而非补齐功能。手势调节是播放器的核心交互，
做成可关的开关，用户误关后只会以为「播放器坏了」。

**改动**：

- `features/settings/settings_page.dart`：移除「播放与视听偏好」分组内的该 `SwitchListTile`
  （含其后的分隔线，保持分组内「表头 → 首个条目」的既有版式）；
- `data/settings/app_service.dart`：同步删除已无引用方的 `AppSettings.enablePlayerGestures`
  —— 字段声明、构造参数、`copyWith` 参数与赋值、加载时对 `pref_player_gestures` 的读取、
  持久化时的写入，共 **7 处**。删除 UI 后该字段便只剩自引用，留着即死代码。
- 老设备上残留的 `pref_player_gestures` 键不再被读取；无迁移动作，也不影响其它偏好。

**保留**：同分组的「长按瞬时加速与触觉震动」—— 该开关**是生效的**，
经 `PlayerPreferences.longPressBoostEnabled` 注入播放器。

**文档**：按维护约定删除 `APP_TODO.md` 第 13 条；同时删除「四、已确认无需处理」整节
（收录了原第 8、9 条的「不做」结论，以及阅读器 / 播放器的若干已确认结论 ——
这些结论自此**无其它书面出处**，需要备查请回溯 git 历史）。

**验收**：`flutter analyze` **0 问题**；`flutter test` **261/261 通过**；
全仓检索确认 `enablePlayerGestures` / `pref_player_gestures` / 「屏幕滑动手势」零残留引用。

### 🐞 AuraPlayer：修复两个「重复回调」缺陷 + 手势期重建归零 + 两处抽取

**背景**：上轮消除播放期逐帧重建后，对播放器做了一次整体排查（通读全部 13 个模块）。
发现两处会导致**宿主重复执行播完逻辑**的真缺陷（同类后果：自动跳集跳两集），
以及剩余最大的一处逐帧重建。

#### ① 修复 `_initializePlayer` 的异步竞态（会让续播定位串集 + 跳两集）

`await controller.initialize()` 之后的代码原先都通过 `_controller` **字段**访问控制器，
而该字段可能已被「下一轮初始化」（宿主切集 / 快速连点选集 / 自动跳集）替换：

- 监听器被挂到**新**控制器上 → 同一控制器两份监听 → 每帧回调跑两遍 →
  **播完回调两次 → 跳两集**；
- 旧调用拿**旧集的断点**去 `seekTo` **新集**的控制器 →
  用户可见「切到第 5 集，画面却停在第 3 集的进度」。

修复：全程持局部引用推进本轮初始化，每个 `await` 之后校验 `identical(controller, _controller)`，
不匹配即丢弃本轮结果 —— **且不在此时 dispose**：取代它的新一轮入口已销毁过旧控制器，
二次释放会触发 `ChangeNotifier` 断言。

顺带堵掉一个隐患：`_initializePlayer` 现在拒绝处理外部托管控制器 ——
否则全屏实例的「重试加载」会接管并销毁宿主正在播放的控制器。

#### ② 修复播完判定可重复触发（抽成 `PlayerCompletionEngine` + 10 个单测）

原判据 `position >= duration` 一旦成立就**持续成立**：库收到平台完成事件后会自行
`pause() + seekTo(duration)`，这些动作又触发新通知且同样满足条件 → 结束回调被逐帧重复调用。
改用平台 `completed` 事件置位的 `VideoPlayerValue.isCompleted` 为主判据
（离散、可靠，重新播放时由库自动清零），位置比较作兜底，并加**每轮播放只上报一次**的闩锁
（状态回到「未播完」时自动释放：重新播放 / seek 回中间 / 循环回片头）。

#### ③ 亮度 / 音量手势改为心跳局部刷新

垂直滑动原先逐帧 `setState` 重建整棵 13 层播放器树（与上轮播放期问题同类）。
现改为：胶囊**首次出现**时 `setState` 一次（Stack 结构变化），其后逐帧只递增 `_gestureTick`，
由亮度遮罩（Stack 第 2 层）、亮度胶囊、音量胶囊三个叶子订阅 —— 与 `_positionTick` 同一套模式。

#### ④ 两处抽取（共约 155 行，主文件省 39 行）

| 新增模块 | 行数 | 职责 |
|---|---|---|
| `player_fullscreen_route.dart` | 36 | 横竖屏 + 沉浸模式 + rootNavigator 路由推入 + 退出恢复；以 `WidgetBuilder` 与播放器**互相解耦**（避免循环 import） |
| `player_progress_slider.dart` | 65 | 进度条视觉：`SliderTheme` / 缩略块尺寸 / 扫光订阅；只接语义回调，不读控制器 |

#### 顺带加固与核对

- `initState` 申请屏幕常亮前判 `widget.active`（休眠实例不应申请，由全屏实例接管）；
- 倍速抽屉与视效抽屉的回调补 `mounted` 守卫（回调路径的 `setState` 一律先判 `mounted`）；
- 核对确认 `PlayerPreferences` **已有**值相等实现（`==` / `hashCode`），无需改动。

**验收**：`flutter analyze` **0 问题**；`flutter test` **261/261 全部通过**（+10）；
`dart run tool/guardrails/check_architecture.dart` **通过**。

`aura_player.dart` **1132 → 1159 行**：本轮以**修缺陷**为主（修复净增 66 行），
抽取共省 39 行，故主文件略有增长；视觉与编排代码另有 101 行落到两个新模块。
（说明记录在此，避免后续只看到行数上升而误判为「越重构越臃肿」。）

### 🧹 AuraPlayer 全屏期间休眠被遮挡实例（原 APP_TODO 第 10 条）

**背景**：全屏是 `Navigator.push` 的独立路由，被遮挡的小屏实例仍留在 widget 树中，
且与全屏实例**监听同一个控制器** —— 每次状态变化两份 `setState` + 两份 build
（Flutter 只对被遮挡路由禁用 ticker，**不**禁用跨路由的 controller 监听回调）。

**先更正原条目的一处错误前提**：原文写「控制器由宿主托管（`AuraPlayer.controller` 为外部传入），
实例 dispose 不会销毁它」—— **不成立**。`video_detail_view` 只传 `playUrl`，
控制器由**小屏实例自己创建并负责销毁**；`controller` 只在**全屏实例**那一侧是外部传入。
若按原方案「全屏期间用小占位替换小屏播放器」实施，小屏实例 `dispose()` 会走
`widget.controller == null → _controller?.dispose()`，**连带销毁全屏正在使用的控制器**，
全屏播放当场中断。

**方案**：不卸载，改为「休眠」—— 新增 `AuraPlayer.active`（默认 `true`）：

| 休眠后行为 | 原因 |
|---|---|
| 不响应帧回调重建、不驱动缓冲扫光 | 隐藏实例不应产生 build 与 ticker 开销 |
| 交还屏幕常亮需求 | 常亮是全局需求计数；休眠实例若继续持有，全屏内暂停后屏幕仍会常亮 |
| **不触发 `onEnded`** | 两实例共用同一控制器，都回调会让宿主的自动跳集**执行两次** |
| 保留 `onProgress` 上报 | 宿主「继续观看」进度不能在全屏期间断档 |

- 唤醒（退出全屏）时按当前真实播放态重新断言常亮，避免播放中屏幕熄灭；
- 宿主两行接入：`active: !_isFullScreen` + `onFullScreenChanged` 回写状态。

**顺带修正一个既有潜在缺陷**：上述「播完回调双份」是**此前就存在**的问题 ——
全屏播放到结尾时两个实例都会调用宿主的跳集回调，而回调内读的是**已更新**的
`_currentEpisodeIndex`，理论上会一次跳过两集。休眠方案使其不再可能发生。
（该缺陷为静态推演发现，**未做真机复现**。）

**新增 1 个组件测试**：`active` 在 `true → false → true` 间切换不抛异常。

### 📖 阅读器横向分页统一选择实现（原 APP_TODO 第 11 条）

- 横向分页由「逐页 `SelectableText`」改为「外层 `SelectionArea` + 内部纯 `Text`」，
  与纵向长卷统一：整屏一个选择容器，不再每页建立独立的 `EditableText`。
- **顺带消除一处手势冲突隐患**：`SelectableText` 自带 `Scrollable`，原先需靠
  `NeverScrollableScrollPhysics` 压制垂直滑动与翻页手势的冲突；改为纯 `Text` 后
  页面内不再存在 `Scrollable`。
- 既有 widget 测试同步更新：原断言 `find.byType(SelectableText)` 改为 `SelectionArea`
  且 **`SelectableText` 期望为零** —— 把「两种模式已统一」这条约定写进测试。

### 🧠 阅读器接入系统内存告警主动释放（原 APP_TODO 第 12 条）

- `ChapterCache` 新增 `evictAllExcept(keep)`：**不受容量上限约束**，只要不在 `keep` 内即释放；
- 阅读器实现 `WidgetsBindingObserver.didHaveMemoryPressure()`（Android `onTrimMemory` /
  iOS `didReceiveMemoryWarning`），收到告警即释放不在屏上的章节正文。
  正文已统一「加载到即落盘」，代价只是一次本地读盘，换来的是不被系统回收整页；
- **保护集合与常规淘汰共用同一份**（抽出 `_cacheProtectSet()`）：纵向长卷正在渲染的章节、
  当前章、预取 / 下载中章节，以及**无远程地址的章节**（详情页直传正文，释放即永久丢失）；
- 顺带抽出「淘汰收尾（同步清空 `_chapters[i].content`）」复用，两条淘汰路径共享同一套语义，
  不会再出现只改一处而漏改另一处。

**新增 4 个单元测试**（`chapter_cache_test.dart`）：释放 `keep` 外全部条目且不受容量约束、
`keep` 覆盖全部条目时零释放、`keep` 为空时全量释放、空缓存调用返回空集合。

**验收**：`flutter analyze` **0 问题**；`flutter test` **251/251 全部通过**（+5）；
`dart run tool/guardrails/check_architecture.dart` **通过**。

### ⚡ AuraPlayer 播放期消除每帧整树重建（原 APP_TODO 第 7 条）

**背景**：`_onControllerUpdate` 由 `video_player` 以视频帧率（约 60 次/秒）触发。
此前实现除手势寻道外，**每帧都 `setState(() {})` 重建整棵 13 层播放器树**，
只为刷新进度条与时间文本 —— 播放期最大的无谓开销。

**方案**：按「数据变化频率」分流。

| 类别 | 处理 |
|---|---|
| 播放位置（每帧变化） | 递增 `_positionTick` 心跳，仅位置类 UI 局部重建 |
| 播放/暂停、缓冲、倍速、总时长、初始化完成（状态变化） | 快照比对，仅在变化时 `setState` 一次 |

- **复用既有通路，未引入新机制**：手势寻道早已用 `ValueNotifier` + `ValueListenableBuilder`
  做局部刷新，且订阅链路已铺到叶子组件（时间文本、进度条、迷你进度条）。
  本轮把该 notifier 语义从「手势预览」泛化为「位置刷新心跳」，并让正常播放帧也驱动它。
- **状态快照抽成 `shared/widgets/player/player_refresh_engine.dart`**：
  `PlayerRefreshEngine.submit(...)` 返回是否需要整树重建；`reset()` 用于切换播放源 /
  重建控制器后清空快照（否则上一份快照来自另一个媒体，会掩盖新控制器首帧的状态变化）。
  首帧恒返回 `true`，保证 `aspectRatio` 等依赖控制器实时值的布局能拿到初值。
- **进度条改为整体订阅心跳**：总时长 / 缓冲比例 / 缓冲态 / 播放位置下沉到
  `_buildProgressSliderBody` 内部计算。此前这些值在上层 `build` 中现算后作为参数传入，
  去掉每帧 `setState` 后若不下沉，缓冲进度会停止更新。
- **拖拽进度条同样零整树重建**：`onChanged` 仅在「拖拽标记翻转」时 `setState` 一次
  （缩略块尺寸与 `_effectiveIsPlaying` 的播放意图判定依赖它），拖拽过程只递增心跳。
- **重命名**：`seekPreviewTick` → `positionTick`（`aura_player.dart` /
  `player_control_bar.dart` / `player_overlays.dart`），与扩展后的语义对齐。

**收益**：播放期整树重建由 ~60 次/秒 降至 **0 次**（状态不变时）；进度条拖拽期同样归零。

**新增 11 个单元测试**（`test/unit/shared/widgets/player/player_refresh_engine_test.dart`）：
首次提交恒触发（含首次调用与 `reset` 后）、**连续 60 帧相同状态全部不重建**、
变化触发一次后同值不再触发、`isPlaying` / `isBuffering` / `playbackSpeed` / `duration` /
`isInitialized` 各自变化均触发、多字段同时变化只触发一次、播放暂停往复每次都触发。

**验收**：`flutter analyze` **0 问题**；`flutter test` **246/246 全部通过**（+11）。
`aura_player.dart` **1150 → 1098 行**（-52）。

**说明**：本条只消除重建次数，**不改任何交互行为**；APP_TODO 原第 8、9 条经评估决定不做
（条目与结论已随该文档的后续清理移除，如需回溯请查 git 历史）。
