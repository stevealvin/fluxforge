...
- **两大阅读翻页模式**（`PageTurnMode`）：
  1. **左右平滑翻页 (`horizontal`)**：标准横向 `PageView` 翻页，并在章末带「下一章衔接页」实现跨章连续翻页；
  2. **上下连续长篇滚动 (`verticalScroll`)**：触底静默续载下一章并追加到同一滚动流，形成真正的无缝长卷；
- **跨章连续阅读（核心体验，必须保持）**：
  - **横向**：本章最后一页之后再追加一页「下一章衔接页」，用户滑入后约 260ms 自动续读下一章（`_advancingChapter` 防抖），依托预取实现无感切换；
  - **纵向**：滚动距离底部不足 480px 时，静默加载下一章正文并追加到同一 `ListView`；同时依据屏幕中线自动同步当前阅读章节（仅检查相邻 3 个章节块，避免逐帧全量遍历），保证进度记录与目录高亮准确；
  - **失败熔断**：纵向续载失败的章节写入 `_verticalFailed` 标记，杜绝滚动过程中对同一章节反复发起无效请求；
- **后台预取（零等待切章）**：
  - 当前章内容加载完成后立即静默预取下一章；横向翻页推进到本章后段（倒数第 2 页）时再次触发；
  - 预取走 `_ensureChapterContent` 纯数据获取通道，不触碰当前 UI 状态，仅写入 `_contentCache` 实现切章秒开，并由 `_prefetching` 集合防止重复请求；
  - 底栏实时展示「已缓存 N 章」，并支持一键手动预取；
- **阅读位置保持（杜绝切换后从头开始）**：
  - 以「本章正文字符偏移量」为统一锚点（`_currentCharOffset`：横向按页累加、纵向按滚动比例换算）；
  - **翻页模式切换**（`_togglePageMode`）、**字号加减**（`_applyTypographyChange`）、**行距调整**（`onChangeStart` 锚定 + `onChangeEnd` 还原）后均自动还原到原阅读位置；
...
- **阅读区三区点击交互（强制规范）**：
  - 屏幕横向三等分：**左 1/3 → 上一页**、**中 1/3 → 呼出/收起控制栏**、**右 1/3 → 下一页**；
  - 实现必须采用 Stack **上层透明热层**（`Positioned.fill` + `Row` 三区 `GestureDetector`），且热层**仅注册 onTap**：
    - 热层必须位于正文之上，才能稳定赢得单击手势竞技场（正文为 `SelectableText`，其内部 tap 识别器会抢走外层点击）；
    - 仅注册 onTap 才能保证长按划词、拖动选择与滑动翻页手势完全不受影响；
    - 严禁在 `SelectableText` 上直接绑定 onTap 做点击分发（既拿不到点击坐标，又会与热层双重响应）；
  - 翻页边界行为：横向模式本章最后一页点击右侧 → 自动无缝续读下一章；本章第一页点击左侧 → 进入上一章并定位到其最后一页；全书首尾必须给出轻提示；
  - 纵向长卷模式：左右点击等价于按屏滚动（0.92 视口高度，260ms `easeOutCubic`），触底仍自动续载下一章。
- **控制栏规范**：
  - 顶部与底部控制栏必须采用**纵向渐变遮罩**（严禁使用不透明色块生硬切断正文）；
  - **已取消「复制章节」功能**：正文本身基于 `SelectableText` 支持长按划词复制，整章复制属冗余入口；
  - 底栏固定四键：`目录` / `已缓存 N 章（点击手动预取）` / `翻页模式` / `排版`；
  - **底栏进度条语义固定为「当前章节内的阅读进度」**：横向翻页模式按「当前页码 / 本章总页数」，纵向长卷模式按「滚动偏移 / 最大滚动距离」；
    拖动即章内跳转（横向 `jumpToPage`、纵向 `jumpTo`），**严禁再用于整本章级跳转**（章级切换由两侧上一章 / 下一章按钮承担）；
    进度条下方必须展示章内进度文案（横向「本章 第 X / Y 页」、纵向「本章 N%」）；
  - 排版抽屉提供护眼底色、翻页模式、字号（A-/A+）与行距四项调节。
- **章节目录规范（左侧抽屉）**：
  - 目录必须以**左侧滑出抽屉**（`Scaffold.drawer`）呈现，严禁使用底部弹层打断阅读；
  - 必须设置 `drawerEnableOpenDragGesture: false`，避免边缘滑动手势与横向翻页 / 左滑冲突，统一由顶栏目录按钮唤醒；
  - **打开时自动定位当前章节**：固定行高 `_catalogItemHeight = 56` + `initialScrollOffset = (当前章索引 - 2) × 行高`，打开即定位且上方保留 2 行上下文；
  - **每项必须展示缓存状态图标**：已缓存 → 绿色对勾；预取中 → 环形进度；未缓存 → 云下载图标（点击即一键缓存该章，成功后需解除纵向续载的失败熔断标记）；
  - 抽屉顶部展示「共 N 章 · 已缓存 M 章」实时统计，当前章节以左侧高亮竖条 + 主色加粗标题标识。
- **空数据与假数据约束（强制）**：
  - 阅读器**严禁注入任何示例/演示章节**（历史版本曾内置《三体》风格示例正文与两章硬编码示例，现已彻底移除）；
  - 未传入任何章节（`chapters` 为空）时必须渲染空态引导页（`_buildEmptyScaffold`），并以 `Navigator.maybePop` 提供返回路径；
  - `build`、`_recalculatePages()`、`_currentCharOffset` 等所有访问 `_chapters[_currentChapterIndex]` 的位置必须具备空数组保护，严禁越界崩溃。
- **进度与持久化**：
  - 退出时自动保存章节索引与阅读进度（通过 `onChapterChanged` 回调交由上层 `PlayHistoryService` 持久化）；
  - 横向模式底部展示「第 N / M 章」与「页码 / 总页数」，读到全书最后一页时提示「已是最后一章」。
