...
void main() {
  ...
  test('media playlist：相对路径分片按清单所在目录绝对化', () {
    ...
    expect(playlist.segmentUrls, equals([
      '${base}seg0.ts',
      'https://cdn.example.com/live/parts/seg1.ts',
    ]));
  ...
  test('AES-128 加密清单把密钥一并纳入本地化资源（解密交给 FFmpeg）', () {
    ...
    expect(playlist.resourceUrls, containsAll(['${base}keys/key.bin', '${base}seg0.ts']));
    // 密钥不计入分片序列
  ...
  test('EXT-X-MAP 初始化段同样纳入本地化资源', () {
    ...
    expect(playlist.resourceUrls, containsAll(['${base}init.mp4', '${base}seg0.m4s']));
  });
  ...
  test('master playlist：变体归入 variantUrls 而非 segmentUrls', () {
    ...
    expect(playlist.variantUrls, equals(['${base}720p.m3u8', '${base}480p.m3u8']));
  ...
  test('rewriteToLocal：映射为空时原样返回；未命中的行不被破坏', () {
    const text = '#EXTM3U\n#EXTINF:6.0,\nseg0.ts\n';
    expect(HlsPlaylistParser.rewriteToLocal(text, baseUri, const {}), equals(text));

    final rewritten = HlsPlaylistParser.rewriteToLocal(
      text,
      baseUri,
